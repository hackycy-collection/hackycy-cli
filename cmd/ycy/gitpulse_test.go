...
func TestTerminalPulseAdapterTranslatesFormsPhasesAndPresentation(t *testing.T) {
	...
	days, cancelled, err := adapter.SelectDays(pulsecommand.DayPrompt{Message: "Select date range:", Options: []pulsecommand.DayChoice{{Value: 1, Label: "Today"}, {Value: 7, Label: "Last 7 days"}}})
	if err != nil || cancelled || days != 7 {
		t.Fatalf("SelectDays() = (%d, %t, %v)", days, cancelled, err)
	}
	authors, cancelled, err := adapter.SelectAuthors(pulsecommand.AuthorPrompt{Message: "Filter by authors:", Options: []pulsecommand.AuthorChoice{{Value: "Ada", Label: "Ada"}, {Value: "Ben", Label: "Ben"}}, InitialValues: []string{"Ada", "Ben"}, Required: true})
	...
	adapter.Present(pulsecommand.Report{CommitCount: 1, Repositories: []pulsecommand.RepositoryReport{{Path: "/workspace/project", Commits: []pulsecommand.Commit{{Date: "2026-08-23 10:00:00", Author: "Ada", Subject: "message"}}}}})
...
func TestTerminalPulseAdapterMapsCancellationAndAutomationInteraction(t *testing.T) {
	...
	if _, cancelled, err := adapter.SelectDays(pulsecommand.DayPrompt{}); err != nil || !cancelled {
	...
	if _, _, err := automation.SelectDays(pulsecommand.DayPrompt{}); !errors.Is(err, errGitPulseRequiresInteractive) {
...
func TestTerminalPulseAdapterBridgesTypedPhaseToTrackedOperation(t *testing.T) {
	...
	reporter, err := adapter.Start(context.Background(), pulsecommand.PhaseScan)
	if err != nil {
		t.Fatalf("Start() error = %v", err)
	}
	reporter.Report(pulsecommand.Phase{Kind: pulsecommand.PhaseScan, State: pulsecommand.PhaseActive, Root: "/workspace", Repository: "/workspace/project", Completed: 1})
...
func TestGitPulseAutomationFailsBeforeReadingInputOrWritingAResult(t *testing.T) {
	workspace := t.TempDir()
	...
	for _, testCase := range []struct {
		...
		{name: "date selection", arguments: []string{"git", "pulse", workspace}},
		{name: "author selection", arguments: []string{"git", "pulse", workspace, "--days", "1"}},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			...
			app, err := newRootCommandForTest("0.0.0-dev", rootTestDependencies{
				...
				GitPulse: newGitPulseHandler(experience),
			})
			if err != nil {
			...
			if outcome.Code != 1 || !errors.Is(outcome.Err, errGitPulseRequiresInteractive) || stdout.Len() != 0 || stderr.String() != "error: git pulse requires an interactive terminal\n" {
...
func TestGitPulsePlainJourneyKeepsPhasesOnStderrAndReportOnStdout(t *testing.T) {
	workspace := t.TempDir()
	initializeStandalonePulseRepository(t, filepath.Join(workspace, "alpha"), "Ada", "ada@example.test", "alpha commit")
	initializeStandalonePulseRepository(t, filepath.Join(workspace, "beta"), "Ben", "ben@example.test", "beta commit")
	stdout := &bytes.Buffer{}
	...
	result, err := newGitPulseHandler(experience)(context.Background(), pulsecommand.Input{Directory: workspace})
	if err != nil || result.Report.CommitCount != 2 {
