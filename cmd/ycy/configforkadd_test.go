...
func TestTerminalForkAddAdapterTranslatesTheOrderedForm(t *testing.T) {
	...
	input, cancelled, err := configfork.PromptAdd(adapter)
	if err != nil || cancelled {
		t.Fatalf("PromptAdd() = (%#v, %t, %v)", input, cancelled, err)
	}
	if got, want := input, (configfork.AddInput{Alias: "work", Host: "gitlab.example", Type: "gitlab", Scheme: "https", Token: "secret-token"}); got != want {
...
func TestTerminalForkAddAdapterMapsTerminalCancellation(t *testing.T) {
	...
	input, cancelled, err := configfork.PromptAdd(adapter)
	if err != nil || !cancelled || input != (configfork.AddInput{}) {
		t.Fatalf("PromptAdd() = (%#v, %t, %v)", input, cancelled, err)
	}
}
...
func TestTerminalForkAddAdapterRoutesPlainPromptAndValidationToDiagnostics(t *testing.T) {
	...
	value, cancelled, err := adapter.Text(configfork.TextPrompt{
...
func TestConfigForkAddAutomationFailsBeforeReadOrWrite(t *testing.T) {
	...
	app, err := newRootCommandForTest("0.0.0-dev", rootTestDependencies{
		...
		ConfigForkAdd: newConfigForkAddHandler(experience),
	})
	if err != nil {
		t.Fatalf("New() error = %v", err)
	}

	outcome := app.Execute(context.Background(), []string{"config", "fork", "add"})
	if outcome.Code != 1 || !errors.Is(outcome.Err, errConfigForkAddRequiresInteractive) {
		t.Fatalf("outcome = %#v", outcome)
	}
	if got, want := stdout.String(), ""; got != want {
		t.Fatalf("stdout = %q, want empty", got)
	}
	if got, want := stderr.String(), "error: config fork add requires an interactive terminal\n"; got != want {
