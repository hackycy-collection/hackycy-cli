...
| `config cm use` | integrated | Core inventory | Integration Commit (this commit): 2026-08-23 focused `GOTOOLCHAIN=go1.26.7 GOWORK=off CGO_ENABLED=0 go test -count=1 ./internal/appconfig ./internal/commands/config/cm ./internal/cliapp ./cmd/ycy`, `make check`, and `make build` passed; module and standalone tests cover exact profile selection, existing/default transition, missing-profile exit 1, success output, parser binding, profile-field/Fork preservation, and only the real CM list/add/use routes. | macOS arm64 host (Darwin 25.5.0); Go 1.26.7 with `CGO_ENABLED=0`; host `build/ycy` SHA-256 `5635f876f188c9fbe7687e0e4cd24d7b7fe176a027ed7c26ea94113642ac8618`; host smoke created two encrypted profiles, selected `work`, and listed only the default marker without API-key output; `make cross-build` produced six nonempty Mach-O/ELF/PE artifacts with matching target CPU inspection. | Release-accepted Artifact Gate and later applicable native evidence | [data scope](issues/12-choose-data-compatibility-mechanisms.md), [module ownership](issues/13-choose-go-module-seams.md), [parity policy](issues/17-choose-corrected-core-command-contracts.md) |
| `config cm set` | integrated | Core inventory | Integration Commit (this commit): 2026-08-23 focused `GOTOOLCHAIN=go1.26.7 GOWORK=off CGO_ENABLED=0 go test -count=1 ./internal/appconfig ./internal/commands/config/cm ./internal/cliapp ./cmd/ycy`, `make check`, and `make build` passed; module, CLI, and standalone tests cover all six keys, legacy numeric forms/prefixes, string normalization/permissiveness, encrypted API-key rotation, missing/unsupported/invalid exits, no false success, and only the real CM leaves. | macOS arm64 host (Darwin 25.5.0); Go 1.26.7 with `CGO_ENABLED=0`; host `build/ycy` SHA-256 `bb4eb7c9aa495a65154dd2376cc5fe1933ee1f886cd6536179140e77d5ab0fc7`; isolated host smoke added and updated an encrypted profile without key disclosure; `make cross-build` produced six nonempty Mach-O/ELF/PE artifacts with matching target CPU inspection. | Release-accepted Artifact Gate and later applicable native evidence | [data scope](issues/12-choose-data-compatibility-mechanisms.md), [module ownership](issues/13-choose-go-module-seams.md), [parity policy](issues/17-choose-corrected-core-command-contracts.md) |
| `config cm remove` | integrated | Core inventory | Integration Commit (this commit): 2026-08-23 focused `GOTOOLCHAIN=go1.26.7 GOWORK=off CGO_ENABLED=0 go test -count=1 ./internal/commands/config/cm ./internal/cliapp ./cmd/ycy`, `make check`, and `make build` passed; module, terminal, CLI, and standalone tests cover confirmation/cancellation, missing, nondefault/default/last removal, persisted default reassignment, unrelated Fork preservation, and only the real CM leaves. | macOS arm64 host (Darwin 25.5.0); Go 1.26.7 with `CGO_ENABLED=0`; isolated host `build/ycy` smoke added two encrypted profiles, removed the default, verified the new default and no secret output; host SHA-256 `619d0d8b1beea49afb62679669ed339ea6974ddc49205d6a52bcaad509b158aa`; `make cross-build` produced six nonempty Mach-O x64/arm64, static ELF x64/arm64, and PE x64/arm64 artifacts with matching format/CPU inspection. | Release-accepted Artifact Gate and later applicable native evidence | [data scope](issues/12-choose-data-compatibility-mechanisms.md), [module ownership](issues/13-choose-go-module-seams.md), [parity policy](issues/17-choose-corrected-core-command-contracts.md) |
| `config cm test` | integrated | Core inventory | Integration worktree: 2026-08-23 focused `GOTOOLCHAIN=go1.26.7 GOWORK=off CGO_ENABLED=0 go test -count=1 ./internal/appconfig ./internal/commands/config/cm ./internal/cliapp ./cmd/ycy`, `make check`, and `make build` passed; local provider tests cover precedence, request fields, DeepSeek, timeout, HTTP, JSON, empty-content, usage, optional CLI profile, success/failure diagnostics, and API-key non-disclosure. | macOS arm64 host (Darwin); Go 1.26.7 with `CGO_ENABLED=0`; host `build/ycy` SHA-256 `02c7750ac267c768a6bf6161598121f205865d4803c816fc0d9e5eb0132c4fba`; standalone local-provider smoke, version/help, and `make cross-build` six-target Mach-O/ELF/PE format/CPU checks passed. | Release-accepted Artifact Gate and native Linux/Windows interactive-provider execution evidence | [module ownership](issues/13-choose-go-module-seams.md), [parity policy](issues/17-choose-corrected-core-command-contracts.md) |
| `rm` | integrated | Core inventory | Integration worktree: 2026-08-23 focused `GOTOOLCHAIN=go1.26.7 GOWORK=off CGO_ENABLED=0 go test -count=1 ./internal/commands/rm ./internal/cliapp ./cmd/ycy`, `make check`, and `make build` passed. Contained module/terminal/standalone evidence covers explicit and smart planning, prompts, depth, force, missing, cancellation, partial failures, result mapping, and no legacy dispatch. | macOS arm64 host (Darwin 25.5.0); Go 1.26.7 with `CGO_ENABLED=0`; current `build/ycy` version/help and validated temporary-root force-removal smoke passed; host SHA-256 `2a69c5318a5e844003d6b3a89df5de6a0feac9736b2912eda623938c9fc5c135`; `make cross-build` produced six nonempty Mach-O x64/arm64, static ELF x64/arm64, and PE x64/arm64 artifacts with matching CPU/format inspection. | Release-accepted Artifact Gate and native Linux/Windows interactive-terminal execution evidence | [module ownership](issues/13-choose-go-module-seams.md), [parity policy](issues/17-choose-corrected-core-command-contracts.md) |
| `run` | integrated | Core inventory | Integration worktree: 2026-08-23 focused `GOTOOLCHAIN=go1.26.7 GOWORK=off CGO_ENABLED=0 go test -count=1 ./internal/commands/run ./internal/cliapp ./cmd/ycy`, `make check`, and `make build` passed. Module, terminal, process-group, CLI, and standalone tests cover project/package/script discovery, lockfile priority, cancellation, help/global-option behavior, the frozen passthrough rejection table, exact child argv/cwd/stdio, missing executable, external Bun selection, child exits, signals, and no-orphan cancellation. | macOS arm64 host (Darwin 25.5.0); Go 1.26.7 with `CGO_ENABLED=0`; current `build/ycy` version/run-help/lone-option-path smoke passed; host SHA-256 `d3d44d8580ce309a421becc2d661f2f1f710acb50358d70573c6884f284c31e1`; `make cross-build` produced six nonempty Mach-O x64/arm64, static ELF x64/arm64, and PE x64/arm64 artifacts with matching CPU/format inspection. | Release-accepted Artifact Gate and native Linux/Windows interactive process/signal execution evidence | [module ownership](issues/13-choose-go-module-seams.md), [parity policy](issues/17-choose-corrected-core-command-contracts.md) |
| `git heat` | integrated | Git inventory | Integration Commit (this commit): 2026-08-23 focused `GOTOOLCHAIN=go1.26.7 GOWORK=off CGO_ENABLED=0 go test -count=1 ./internal/commands/git/heat ./internal/cliapp ./cmd/ycy`, `make check`, and `make build` passed; module, CLI, native disposable-repository, process, and standalone tests cover option/range compatibility, path-safe Git parsing, aggregation/reporting, repository kinds, failures, signal cleanup, and the sole real Git leaf. | macOS arm64 host (Darwin 25.5.0); Go 1.26.7 with `CGO_ENABLED=0`; host `build/ycy` SHA-256 `77b37bce11d4c7bfc7426f93831c513c7adebf32d0cdf0fad196c04f49c4d3a4`; isolated host version/help/nested-repository smoke and controlled SIGTERM exit 143 with no child remained; `make cross-build` produced six nonempty Mach-O x64/arm64, static ELF x64/arm64, and PE x64/arm64 artifacts with matching format/CPU inspection. | Release-accepted Artifact Gate and native Linux/Windows external-Git and terminal evidence | [module ownership](issues/13-choose-go-module-seams.md), [parity policy](issues/17-choose-corrected-core-command-contracts.md) |
