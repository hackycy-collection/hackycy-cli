...
- Current Rich finite-command visual contract is variant B, **Ops Console**:
  a top command/status bar, safe metadata row, aligned `STATE / PHASE / DETAIL`
  table, and an active form or result region. It has amber primary and cyan
  accent colors with symbol-paired state labels, a Huh bottom focus rule, no
  persistent left rail, and a single-column degradation on narrow or short
  terminals. `diff`, `fs`, `tunnel connect`, and `tunnel server` remain
  line-oriented Lifecycle Logs and do not use the full-screen Console.
...
- [Prototype the vivid terminal visual system](issues/05-prototype-vivid-terminal-visual-system.md): adopt Ops Console (B): command/status bar, safe metadata row, aligned `STATE / PHASE / DETAIL` table, active form/result region, amber primary with cyan accent, symbol-paired states, Huh bottom focus rule, no persistent rail, and compact post-AltScreen transcript replay; A/C remain comparison variants.
...
- [Choose the config fork add terminal experience](issues/10-choose-config-fork-add-presentation.md): keep the five-step Huh form and validation contract, add Ops Console field/save status-table phases, redact credentials and unsafe host details, preserve silent alias overwrite, and submit exactly one outcome result.
...
- [Choose the config cm add terminal experience](issues/13-choose-config-cm-add-presentation.md): preserve the four-field Huh form and validation/overwrite semantics while adding Ops Console collection and save status-table phases, safe profile/URL projections, redacted cancellation transcripts, and one-shot success/failure outcomes.
