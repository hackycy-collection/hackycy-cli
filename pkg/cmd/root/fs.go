...
func (app *App) registerFS(root *cobra.Command, configureLogging func() error) {
	...
	command := &cobra.Command{
		...
		RunE: func(command *cobra.Command, arguments []string) error {
			...
			idleDays, err := parseFSIdleDays(fsValue(command, "session-idle-days", sessionIdleDays, app.factory.Environment("YCY_FS_SESSION_IDLE_DAYS"), "7"))
			if err != nil {
				return err
			}
			chunked, err := parseFSBool(fsValue(command, "chunked-upload", strconv.FormatBool(chunkedUploads), app.factory.Environment("YCY_FS_CHUNKED_UPLOAD"), "false"))
			if err != nil {
				return err
			}
			chunkSize, err := parseFSChunkSize(fsValue(command, "upload-chunk-size", uploadChunkSize, app.factory.Environment("YCY_FS_UPLOAD_CHUNK_SIZE_MIB"), "8"))
			if err != nil {
				return err
			}
			directory := "."
			...
			sessions := fsValue(command, "session-dir", sessionDirectory, app.factory.Environment("YCY_FS_SESSION_DIR"), "")
