...
This directory contains the complete implementation of `ycy diff`: a read-only directory comparison engine, a same-origin HTTP server, and an embedded React application.
...
The previous snapshot remains readable while refresh runs. Cancellation or failure never partially replaces it. Publishing a new snapshot invalidates browser tabs and cached content because every API read is bound to a snapshot ID.
...
The server does not generate Git patches. Text content is sent to `@pierre/diffs` in browser workers. Bundled hashed assets are immutable-cacheable; the HTML shell remains `no-store`.
