...
func TestScanRepositoriesFindsDirectAndNestedGitDirectories(t *testing.T) {
	...
	var found []string
	repositories, err := ScanRepositories(context.Background(), root, osDirectoryReader{}, func(path string) {
...
func TestScanRepositoriesSkipsExcludedDirectoriesAndSymlinks(t *testing.T) {
	...
	repositories, err := ScanRepositories(context.Background(), root, osDirectoryReader{}, nil, nil)
	if err != nil {
		t.Fatalf("ScanRepositories() error = %v", err)
	}
...
func TestScanRepositoriesYieldsAfterEachFullScanBatch(t *testing.T) {
	...
	yields := 0
	repositories, err := ScanRepositories(context.Background(), root, osDirectoryReader{}, nil, func() {
		yields++
	})
	if err != nil {
		t.Fatalf("ScanRepositories() error = %v", err)
