...
type serverFRPRuntimeEnsurer func(context.Context, string, FRPArtifact) (FRPRuntimePaths, error)
...
func NewServerRuntime(ctx context.Context, options ServerRuntimeOptions) (*ServerRuntime, error) {
	...
	lock, err := acquireStateDirectoryLock(dataDirectory)
	if err != nil {
		return nil, err
	}
	...
	frpPaths := frpRuntimePaths(runtime.frpDirectory, runtime.frpArtifact.Target)
	runtime.supervisor, err = NewFRPSupervisor(FRPSupervisorOptions{
...
func resolveServerFRPRuntime(options ServerRuntimeOptions) (FRPArtifact, string, serverFRPRuntimeEnsurer, error) {
	...
	if err != nil {
		return FRPArtifact{}, "", nil, fmt.Errorf("resolve managed FRP runtime directory: %w", err)
	}
...
func (runtime *ServerRuntime) ensureManagedFRPRuntime(ctx context.Context) error {
	...
	expected := frpRuntimePaths(runtime.frpDirectory, runtime.frpArtifact.Target)
	if paths != expected {
