...
func TestServerAgentConnectionRejectsIncompatibleHello(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "frps stopped after authorization", message: validHello, configure: func(availability *serverAgentTestFRPSAvailability) { availability.set(FRPProcessStopped) }, closeCode: serverAgentCloseFRPSUnavailable},
	} {
		t.Run(test.name, func(t *testing.T) {
...
func TestServerAgentConnectionPresentsDesiredStateOnlyAfterWelcome(t *testing.T) {
	...
	welcome, ok := frames[0].(AgentWelcome)
	...
	desired, ok := frames[1].(DesiredState)
	if !ok || desired.Type != "desired_state" || desired.TunnelProtocolVersion != TunnelProtocolVersion || desired.Snapshot.Revision != 2 || len(desired.Snapshot.Tunnels) != 2 {
...
func TestServerAgentConnectionProjectsApplyResultRuntimeErrors(t *testing.T) {
	...
	if got := connection.gateway.State(connection.ClientID()); got.ProcessState != FRPProcessStopped || got.LastError == nil || got.LastError.Code != "ACTIVATION_FAILED" || got.LastError.Message != "candidate exited" || got.LastError.Revision == nil || *got.LastError.Revision != 1 {
...
func TestServerAgentConnectionProjectsDefaultApplyFailure(t *testing.T) {
	...
	if got := connection.gateway.State(connection.ClientID()); got.ProcessState != FRPProcessRunning || got.LastError == nil || got.LastError.Revision == nil || *got.LastError.Revision != 1 {
...
func TestServerAgentConnectionProjectsProcessStateWithoutDurableMutation(t *testing.T) {
	...
	if got := connection.gateway.State(connection.ClientID()); got.ConnectionState != ServerClientConnected || got.ProcessState != FRPProcessRunning || got.LastError != nil {
	...
	if got := connection.gateway.State(connection.ClientID()); got.ConnectionState != ServerClientConnected || got.ProcessState != FRPProcessConfigurationFailed || got.LastError == nil || got.LastError.Code != "ACTIVATION_FAILED" || got.LastError.Message != "candidate exited" || got.LastError.Revision == nil || *got.LastError.Revision != 0 {
	...
	connection.Close()
	if got := connection.gateway.State(connection.ClientID()); got.ConnectionState != ServerClientDisconnected || got.ProcessState != FRPProcessConfigurationFailed || got.LastError == nil || got.LastError.Revision == nil || *got.LastError.Revision != 0 {
