...
func serverTunnelPatchProtocol(parent map[string]json.RawMessage) (*TunnelProtocol, error) {
	value, err := serverTunnelOptionalString(parent, "protocol", false)
	if err != nil || value == nil {
		return nil, err
	}
	protocol := TunnelProtocol(*value)
...
func serverTunnelPatchBandwidthLimit(parent map[string]json.RawMessage) (*TunnelPatchValue[*TunnelBandwidthLimit], error) {
	raw, found := parent["bandwidthLimit"]
	if !found {
		return nil, nil
	}
	if serverTunnelNull(raw) {
		return &TunnelPatchValue[*TunnelBandwidthLimit]{}, nil
	}
	value, err := serverTunnelBandwidthLimit(map[string]json.RawMessage{"bandwidthLimit": raw})
	if err != nil {
		return nil, err
	}
	return &TunnelPatchValue[*TunnelBandwidthLimit]{Value: value}, nil
}
...
func serverTunnelPatchHeaders(parent map[string]json.RawMessage, key string) (*[]TunnelHeader, error) {
	if _, found := parent[key]; !found {
		return nil, nil
	}
