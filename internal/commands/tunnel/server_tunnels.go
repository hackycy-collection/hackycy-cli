...
type TunnelTransportOptionsPatchInput struct {
	UseEncryption        *bool
	UseCompression       *bool
	BandwidthLimit       *TunnelPatchValue[*TunnelBandwidthLimit]
...
type TunnelTransportOptionsInput struct {
	UseEncryption        *bool
	UseCompression       *bool
	BandwidthLimit       *TunnelBandwidthLimit
...
func (plane *ServerControlPlane) CreateTunnel(ctx context.Context, clientID string, input TunnelMutationInput) (TunnelDefinition, error) {
	tunnelID, err := randomUUID(plane.random)
	if err != nil {
		return TunnelDefinition{}, fmt.Errorf("generate Tunnel Definition ID: %w", err)
	}
...
func (plane *ServerControlPlane) UpdateTunnel(ctx context.Context, tunnelID string, patch TunnelPatchInput) (TunnelDefinition, error) {
	timestamp := formatServerTimestamp(plane.now())
	result, err := withImmediateTransaction(ctx, plane.database, func(connection *sql.Conn) (struct {
		tunnel ServerTunnel
		owner  string
...
func (plane *ServerControlPlane) ListTunnels(ctx context.Context, clientID string) ([]TunnelDefinition, error) {
	...
	defer rows.Close()
	tunnels := make([]TunnelDefinition, 0)
	for rows.Next() {
...
func (plane *ServerControlPlane) Snapshot(ctx context.Context, clientID string) (TunnelSnapshot, error) {
	...
	return TunnelSnapshot{ClientKey: client.ID, Revision: client.DesiredRevision, Tunnels: tunnels}, nil
}
...
func (plane *ServerControlPlane) tunnelValuesWithOptions(ctx context.Context, connection *sql.Conn, input TunnelMutationInput, protocol TunnelProtocol, options TunnelOptions) (normalizedTunnelValues, error) {
...
func tunnelMutationForPatch(current ServerTunnel, patch TunnelPatchInput) TunnelMutationInput {
	...
	} else if current.Protocol == TunnelProtocolHTTP {
		input.CustomDomains = append([]string(nil), current.CustomDomains...)
	}
	if patch.Location != nil {
		input.Location = patch.Location.Value
	} else if current.Protocol == TunnelProtocolHTTP && current.Location != nil {
...
func (plane *ServerControlPlane) availablePort(ctx context.Context, connection *sql.Conn, protocol TunnelProtocol) (int64, error) {
...
func encodedTunnelValues(values normalizedTunnelValues) (string, any, any, error) {
	...
	if values.protocol != TunnelProtocolHTTP {
...
func scanTunnel(scanner tunnelScanner) (ServerTunnel, error) {
	...
	tunnel.Protocol = TunnelProtocol(protocol)
...
func normalizeTunnelProtocol(protocol TunnelProtocol) (TunnelProtocol, error) {
...
func normalizeTunnelOptions(protocol TunnelProtocol, input *TunnelOptionsInput) (TunnelOptions, error) {
	...
	if protocol != TunnelProtocolHTTP {
		return TunnelOptions{Transport: transport, HealthCheck: health}, nil
	}
	...
	return TunnelOptions{Transport: transport, HealthCheck: health, HTTP: httpOptions}, nil
}

func normalizeTunnelPatchOptions(protocol TunnelProtocol, input *TunnelOptionsPatchInput, current TunnelOptions) (TunnelOptions, error) {
	...
	if protocol != TunnelProtocolHTTP {
		return TunnelOptions{Transport: transport, HealthCheck: healthCheck}, nil
	}
	...
	return TunnelOptions{Transport: transport, HealthCheck: healthCheck, HTTP: httpOptions}, nil
}

func cloneTunnelTransportOptions(source TunnelTransportOptions) TunnelTransportOptions {
	copy := source
...
func cloneTunnelHealthCheck(source *TunnelHealthCheck) *TunnelHealthCheck {
	if source == nil {
		return nil
	}
	copy := *source
...
func defaultTunnelHTTPOptions() *TunnelHTTPOptions {
	return &TunnelHTTPOptions{RequestHeaders: []TunnelHeader{}, ResponseHeaders: []TunnelHeader{}}
}

func cloneTunnelHTTPOptions(source *TunnelHTTPOptions) *TunnelHTTPOptions {
	if source == nil {
...
func cloneTunnelHeaders(source []TunnelHeader) []TunnelHeader {
	if len(source) == 0 {
		return []TunnelHeader{}
	}
	return append([]TunnelHeader(nil), source...)
}

func normalizeTunnelBandwidthLimit(input *TunnelBandwidthLimit) (*TunnelBandwidthLimit, error) {
	if input == nil {
		return nil, nil
	}
...
func applyTunnelHTTPOptionsPatch(current *TunnelHTTPOptions, patch *TunnelHTTPOptionsPatchInput) (*TunnelHTTPOptions, error) {
...
func normalizeTunnelBasicAuthPatch(input *TunnelBasicAuthPatchInput, current *TunnelBasicAuth) (*TunnelBasicAuth, error) {
	if input == nil {
		return nil, nil
	}
	...
	return &TunnelBasicAuth{Username: username, Password: password}, nil
}
...
func normalizeTunnelTransportOptions(input *TunnelTransportOptionsInput) (TunnelTransportOptions, error) {
	...
	if input.BandwidthLimit != nil {
		if math.IsNaN(input.BandwidthLimit.Value) || math.IsInf(input.BandwidthLimit.Value, 0) || input.BandwidthLimit.Value <= 0 || (input.BandwidthLimit.Unit != "KB" && input.BandwidthLimit.Unit != "MB") || (input.BandwidthLimit.Mode != "client" && input.BandwidthLimit.Mode != "server") {
			return TunnelTransportOptions{}, serverDomainError("INVALID_TUNNEL", "Bandwidth limit is invalid")
		}
	...
	if input.ProxyProtocolVersion != nil {
		if *input.ProxyProtocolVersion != "v1" && *input.ProxyProtocolVersion != "v2" {
			return TunnelTransportOptions{}, serverDomainError("INVALID_TUNNEL", "Proxy Protocol version is invalid")
		}
...
func normalizeTunnelHealthCheck(input *TunnelHealthCheckInput) (*TunnelHealthCheck, error) {
	if input == nil {
		return nil, nil
	}
	...
	if input.Type == "tcp" {
		return &TunnelHealthCheck{Type: "tcp", IntervalSeconds: input.IntervalSeconds, TimeoutSeconds: input.TimeoutSeconds, MaxFailed: input.MaxFailed}, nil
	}
	...
	return &TunnelHealthCheck{Type: "http", Path: &path, IntervalSeconds: input.IntervalSeconds, TimeoutSeconds: input.TimeoutSeconds, MaxFailed: input.MaxFailed, Headers: headers}, nil
}

func normalizeTunnelHTTPOptions(input *TunnelHTTPOptionsInput) (*TunnelHTTPOptions, error) {
	options := &TunnelHTTPOptions{RequestHeaders: []TunnelHeader{}, ResponseHeaders: []TunnelHeader{}}
	if input == nil {
		return options, nil
	}
	if input.BasicAuth != nil {
		...
		options.BasicAuth = &TunnelBasicAuth{Username: username, Password: input.BasicAuth.Password}
	}
...
func normalizeTunnelHeaders(headers []TunnelHeader, label string) ([]TunnelHeader, error) {
	if len(headers) > 32 {
		return nil, serverDomainError("INVALID_TUNNEL", label+" accepts at most 32 headers")
	}
	normalized := make([]TunnelHeader, 0, len(headers))
	...
	for _, header := range headers {
		...
		seen[key] = struct{}{}
		normalized = append(normalized, TunnelHeader{Name: name, Value: value})
	}
