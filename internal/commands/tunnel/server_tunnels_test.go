...
func TestServerControlPlaneReservesHTTPRoutesTransactionally(t *testing.T) {
	...
	if tunnel.Protocol != TunnelProtocolHTTP || strings.Join(tunnel.CustomDomains, ",") != "app.example.com,alias.example.com" || tunnel.Location == nil || *tunnel.Location != location || tunnel.Enabled {
		t.Fatalf("HTTP Tunnel = %#v", tunnel)
	}
	_, err = plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"alias.example.com"}, Location: &location, LocalPort: 3001})
	...
	if _, err := plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"app.example.com"}, Location: &siblingLocation, LocalPort: 3002}); err != nil {
		t.Fatalf("CreateTunnel(sibling) error = %v", err)
	}
	catchAll, err := plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"app.example.com"}, LocalPort: 3003})
	...
	if _, err := plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"app.example.com"}, Location: &location, LocalPort: 3004}); err != nil {
...
func TestServerControlPlaneAllocatesPortReservationsAndRecordsRevisions(t *testing.T) {
	...
	tcp, err := plane.CreateTunnel(ctx, first.ID, TunnelMutationInput{Protocol: TunnelProtocolTCP, LocalPort: 5432})
	if err != nil {
		t.Fatalf("CreateTunnel(tcp) error = %v", err)
	}
	udp, err := plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolUDP, LocalPort: 53})
	if err != nil {
		t.Fatalf("CreateTunnel(udp) error = %v", err)
	}
	nextTCP, err := plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolTCP, LocalPort: 5433})
	...
	port := int64(20001)
	_, err = plane.CreateTunnel(ctx, first.ID, TunnelMutationInput{Protocol: TunnelProtocolTCP, ServerPort: &port, LocalPort: 1})
	assertServerDomainCode(t, err, "RESOURCE_RESERVED")
	...
	tunnel, err := plane.CreateTunnel(context.Background(), client.ID, TunnelMutationInput{
		...
		Options: &TunnelOptionsInput{
			Transport: &TunnelTransportOptionsInput{
				...
				BandwidthLimit:       &TunnelBandwidthLimit{Value: 2, Unit: "MB", Mode: "server"},
				ProxyProtocolVersion: &proxyProtocol,
			},
			HealthCheck: &TunnelHealthCheckInput{Type: "http", Path: stringPointer("/health"), IntervalSeconds: 10, TimeoutSeconds: 3, MaxFailed: 2, Headers: []TunnelHeader{{Name: "X-Probe", Value: "ycy"}}},
			HTTP: &TunnelHTTPOptionsInput{
				BasicAuth:         &TunnelBasicAuth{Username: "operator", Password: password},
				HostHeaderRewrite: &hostRewrite,
				RequestHeaders:    []TunnelHeader{{Name: "X-Forwarded-By", Value: "ycy"}},
				ResponseHeaders:   []TunnelHeader{{Name: "X-Tunnel", Value: "ticket"}},
			},
		},
	})
	if err != nil {
...
func TestServerControlPlanePatchesTunnelReservationsAndRevisionsAtomically(t *testing.T) {
	...
	firstLocation := "/first"
	tunnel, err := plane.CreateTunnel(ctx, first.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"app.example.com"}, Location: &firstLocation, LocalPort: 3000})
	if err != nil {
		t.Fatalf("CreateTunnel(first) error = %v", err)
	}
	reservedLocation := "/reserved"
	if _, err := plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"app.example.com"}, Location: &reservedLocation, LocalPort: 3001}); err != nil {
...
func TestServerControlPlanePatchesTypedOptionsWithoutLeakingOrDroppingOmittedFields(t *testing.T) {
	...
	tunnel, err := plane.CreateTunnel(ctx, client.ID, TunnelMutationInput{
		Protocol: TunnelProtocolHTTP, CustomDomains: []string{"options.example.com"}, LocalPort: 9001,
		Options: &TunnelOptionsInput{
			...
			HealthCheck: &TunnelHealthCheckInput{Type: "http", Path: stringPointer("/health"), IntervalSeconds: 10, TimeoutSeconds: 3, MaxFailed: 2, Headers: []TunnelHeader{{Name: "X-Probe", Value: "ycy"}}},
			HTTP:        &TunnelHTTPOptionsInput{BasicAuth: &TunnelBasicAuth{Username: "operator", Password: password}, HostHeaderRewrite: &hostRewrite, RequestHeaders: []TunnelHeader{{Name: "X-Request", Value: "first"}}, ResponseHeaders: []TunnelHeader{{Name: "X-Response", Value: "first"}}},
		},
	})
	if err != nil {
		t.Fatalf("CreateTunnel() error = %v", err)
	...
	patched, err = plane.UpdateTunnel(ctx, tunnel.ID, TunnelPatchInput{Options: &TunnelOptionsPatchInput{
		Transport:   &TunnelTransportOptionsPatchInput{BandwidthLimit: &TunnelPatchValue[*TunnelBandwidthLimit]{}, ProxyProtocolVersion: &TunnelPatchValue[*string]{}},
...
func TestServerControlPlanePatchSwitchesProtocolAndReleasesHTTPReservation(t *testing.T) {
	...
	tunnel, err := plane.CreateTunnel(ctx, first.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"released.example.com"}, LocalPort: 3000})
	if err != nil {
		t.Fatalf("CreateTunnel() error = %v", err)
	}
	...
	if patched.Protocol != TunnelProtocolTCP || patched.ServerPort == nil || patched.Options.HTTP != nil || len(patched.CustomDomains) != 0 || patched.Location != nil {
		t.Fatalf("UpdateTunnel(protocol) = %#v", patched)
	}
	if _, err := plane.CreateTunnel(ctx, second.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"released.example.com"}, LocalPort: 3001}); err != nil {
		t.Fatalf("CreateTunnel(released reservation) error = %v", err)
	}
}
