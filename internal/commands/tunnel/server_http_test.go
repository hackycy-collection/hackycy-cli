...
func TestServerHTTPHandlerReadsScopedClientAndTunnelViewsWithoutCredentials(t *testing.T) {
	...
	if _, err := plane.CreateTunnel(context.Background(), client.ID, TunnelMutationInput{
		...
		Options: &TunnelOptionsInput{HTTP: &TunnelHTTPOptionsInput{
			BasicAuth: &TunnelBasicAuth{Username: "operator", Password: "secret-value"},
		}},
	}); err != nil {
	...
	if view.ID != client.ID || view.Owner != (serverHTTPClientOwner{ID: alice.ID, Username: "alice"}) || view.Runtime.ConnectionState != ServerClientDisconnected || view.Runtime.ProcessState != FRPProcessStopped || view.TunnelCounts != (serverHTTPTunnelCounts{Total: 1, Enabled: 1, Pending: 1}) {
...
func TestServerTunnelPresentationStateForReadViews(t *testing.T) {
	...
	running := ServerClientRuntimeState{ConnectionState: ServerClientConnected, ProcessState: FRPProcessRunning}
	for _, test := range []struct {
		name    string
		...
		{name: "disabled", tunnel: TunnelDefinition{Enabled: false}, client: client, runtime: running, want: ServerTunnelDisabled},
		{name: "current error", tunnel: TunnelDefinition{Enabled: true}, client: client, runtime: ServerClientRuntimeState{ProcessState: FRPProcessRunning, LastError: &StructuredRuntimeError{Code: "CONFIGURATION_FAILED", Revision: &revision}}, want: ServerTunnelError},
		{name: "unapplied revision", tunnel: TunnelDefinition{Enabled: true}, client: TrustedTunnelClient{DesiredRevision: revision, LastAppliedRevision: revision - 1}, runtime: running, want: ServerTunnelPending},
		{name: "stopped process", tunnel: TunnelDefinition{Enabled: true}, client: client, runtime: ServerClientRuntimeState{ProcessState: FRPProcessStopped}, want: ServerTunnelPending},
		{name: "applied", tunnel: TunnelDefinition{Enabled: true}, client: client, runtime: running, want: ServerTunnelApplied},
	} {
		t.Run(test.name, func(t *testing.T) {
...
func TestServerHTTPHandlerDeletesOwnedClientAndReleasesTunnelReservations(t *testing.T) {
	...
	if _, err := plane.CreateTunnel(context.Background(), client.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"released.example.test"}, LocalPort: 3000}); err != nil {
		t.Fatalf("CreateTunnel() error = %v", err)
	}
	handler, err := NewServerHTTPHandler(ServerHTTPOptions{Sessions: sessions, Accounts: accounts, ControlPlane: plane})
	if err != nil {
	...
	if _, err := plane.CreateTunnel(context.Background(), replacement.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"released.example.test"}, LocalPort: 3000}); err != nil {
...
func TestServerHTTPHandlerCreatesOwnedTypedTunnels(t *testing.T) {
	...
	if httpBody.Version != 1 || httpBody.Tunnel.ID == "" || httpBody.Tunnel.Label != "Public app" || httpBody.Tunnel.Protocol != TunnelProtocolHTTP || len(httpBody.Tunnel.CustomDomains) != 1 || httpBody.Tunnel.CustomDomains[0] != "routes.example.test" || httpBody.Tunnel.Location == nil || *httpBody.Tunnel.Location != "/service" || httpBody.Tunnel.ServerPort != nil || httpBody.Tunnel.LocalHost != "127.0.0.1" || httpBody.Tunnel.LocalPort != 3000 || !httpBody.Tunnel.Enabled || httpBody.Tunnel.State != ServerTunnelPending {
	...
	if tcpBody.Tunnel.Protocol != TunnelProtocolTCP || tcpBody.Tunnel.ServerPort == nil || *tcpBody.Tunnel.ServerPort != 20002 || tcpBody.Tunnel.Enabled || tcpBody.Tunnel.State != ServerTunnelDisabled || strings.Contains(tcpResponse.Body.String(), "customDomains") {
	...
	if udpBody.Tunnel.Protocol != TunnelProtocolUDP || udpBody.Tunnel.ServerPort == nil || *udpBody.Tunnel.ServerPort != 20000 || !udpBody.Tunnel.Enabled || udpBody.Tunnel.State != ServerTunnelPending || strings.Contains(udpResponse.Body.String(), "customDomains") {
...
func TestServerHTTPHandlerPatchesOwnedTypedTunnels(t *testing.T) {
	...
	tunnel, err := plane.CreateTunnel(context.Background(), client.ID, TunnelMutationInput{
		Protocol: TunnelProtocolHTTP, CustomDomains: []string{"patch.example.test"}, Location: &location, LocalPort: 3000,
		Options: &TunnelOptionsInput{
			Transport:   &TunnelTransportOptionsInput{UseEncryption: boolPointer(true), BandwidthLimit: &TunnelBandwidthLimit{Value: 2, Unit: "MB", Mode: "server"}, ProxyProtocolVersion: &proxyProtocol},
			HealthCheck: &TunnelHealthCheckInput{Type: "http", Path: stringPointer("/health"), IntervalSeconds: 10, TimeoutSeconds: 3, MaxFailed: 2, Headers: []TunnelHeader{{Name: "X-Probe", Value: "initial"}}},
			HTTP:        &TunnelHTTPOptionsInput{BasicAuth: &TunnelBasicAuth{Username: "operator", Password: password}, HostHeaderRewrite: &hostRewrite, RequestHeaders: []TunnelHeader{{Name: "X-Request", Value: "initial"}}, ResponseHeaders: []TunnelHeader{{Name: "X-Response", Value: "initial"}}},
		},
	})
	if err != nil {
		t.Fatalf("CreateTunnel() error = %v", err)
	...
	if _, err := plane.CreateTunnel(context.Background(), reservedClient.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"patch.example.test"}, Location: &reservedLocation, LocalPort: 3002}); err != nil {
	...
	if protocolBody.Tunnel.Protocol != TunnelProtocolTCP || protocolBody.Tunnel.ServerPort == nil || *protocolBody.Tunnel.ServerPort != 20001 || len(protocolBody.Tunnel.CustomDomains) != 0 || strings.Contains(protocolResponse.Body.String(), "customDomains") {
...
func TestServerHTTPHandlerDeletesOwnedTunnelAndReleasesReservation(t *testing.T) {
	...
	tunnel, err := plane.CreateTunnel(context.Background(), client.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"released-tunnel.example.test"}, LocalPort: 3000})
	if err != nil {
		t.Fatalf("CreateTunnel() error = %v", err)
	}
	handler, err := NewServerHTTPHandler(ServerHTTPOptions{Sessions: sessions, Accounts: accounts, ControlPlane: plane})
	...
	if _, err := plane.CreateTunnel(context.Background(), replacement.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"released-tunnel.example.test"}, LocalPort: 3001}); err != nil {
		t.Fatalf("CreateTunnel(released reservation) error = %v", err)
	}
}
...
func TestServerHTTPHandlerPreviewsScopedTunnelImportWithoutPersistingState(t *testing.T) {
	...
	if !ok || first["id"] != "proxy-0-location-0" || first["protocol"] != string(TunnelProtocolHTTP) || first["location"] != "/api" || basicAuth["username"] != "operator" || basicAuth["passwordConfigured"] != true || basicAuth["password"] != nil {
...
func TestServerHTTPHandlerImportsSelectedScopedTunnelsAtomically(t *testing.T) {
	...
	if _, err := plane.CreateTunnel(context.Background(), conflictingClient.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"conflict.example.test"}, Location: &conflictLocation, LocalPort: 3001}); err != nil {
	...
	basicAuth, ok := httpOptions["basicAuth"].(map[string]any)
	if !ok || first["protocol"] != string(TunnelProtocolHTTP) || first["enabled"] != false || first["state"] != string(ServerTunnelDisabled) || basicAuth["username"] != "operator" || basicAuth["passwordConfigured"] != true || basicAuth["password"] != nil {
	...
	storedTunnels, err := plane.ListTunnels(context.Background(), client.ID)
	var storedHTTP *TunnelDefinition
	for index := range storedTunnels {
		if storedTunnels[index].Protocol == TunnelProtocolHTTP {
...
func TestServerHTTPHandlerServesScopedStateAndRedactsDeploymentSecrets(t *testing.T) {
	...
	handler, err := NewServerHTTPHandler(ServerHTTPOptions{
		...
		Runtime: serverHTTPTestClientRuntime{states: map[string]ServerClientRuntimeState{
			aliceCurrent.ID: {ConnectionState: ServerClientConnected, ProcessState: FRPProcessRunning},
			alicePending.ID: {ConnectionState: ServerClientDisconnected, ProcessState: FRPProcessStopped},
			bobFailed.ID: {
				ConnectionState: ServerClientConnected,
				ProcessState:    FRPProcessRunning,
				LastError:       &StructuredRuntimeError{Code: "APPLY_FAILED", Message: "client configuration failed", Revision: &failedRevision},
			},
		}},
		ServerState: serverHTTPTestStateProvider{state: ServerHTTPState{
			FRPS: FRPSupervisorState{
				State: FRPProcessConfigurationFailed,
				PID:   &prefix,
				Error: &StructuredRuntimeError{Code: "FRPS_FAILED", Message: "address already in use"},
			},
	...
	type serverView struct {
		FRPS struct {
			...
			PID   *int                    `json:"pid"`
			Error *StructuredRuntimeError `json:"error"`
		} `json:"frps"`
...
func TestServerHTTPHandlerControlsManagedFRPSForAdministrators(t *testing.T) {
	...
	handler, err := NewServerHTTPHandler(ServerHTTPOptions{
		...
		FRPS:         controller,
		ServerState: serverHTTPTestStateProvider{state: ServerHTTPState{
			FRPS: FRPSupervisorState{State: FRPProcessStopped},
			Settings: ServerHTTPServerSettings{
	...
	for _, action := range []ServerFRPSAction{ServerFRPSActionStart, ServerFRPSActionStop, ServerFRPSActionRestart} {
		...
		if body.Version != 1 || body.Server.FRPS.State != FRPProcessStopped || body.Server.Settings.AdminUser != "admin" {
...
func TestServerHTTPHandlerServesNonUpgradeAgentBearerProbe(t *testing.T) {
	...
	plane := openServerControlPlane(t, state)
	availability := &serverAgentTestFRPSAvailability{state: FRPProcessRunning}
	...
	availability.set(FRPProcessStopped)
...
func TestServerHTTPHandlerKeepsAwaitingHelloAgentAliveWhenItPongs(t *testing.T) {
	...
	if got := gateway.State(client.ID); got != (ServerClientRuntimeState{ConnectionState: ServerClientConnected, ProcessState: FRPProcessStopped}) {
...
func TestServerHTTPHandlerClosesAgentWhenPongIsMissing(t *testing.T) {
	...
	deadline := time.Now().Add(time.Second)
	for {
		if got := gateway.State(client.ID); got == (ServerClientRuntimeState{ConnectionState: ServerClientDisconnected, ProcessState: FRPProcessStopped}) {
			break
		} else if time.Now().After(deadline) {
...
func TestServerHTTPHandlerSendsWelcomeAfterValidAgentHello(t *testing.T) {
	...
	if desired.Type != "desired_state" || desired.TunnelProtocolVersion != TunnelProtocolVersion || desired.Snapshot.ClientKey != client.ID || desired.Snapshot.Revision != 1 || len(desired.Snapshot.Tunnels) != 1 {
	...
	if revoke.Type != "revoke" || revoke.TunnelProtocolVersion != TunnelProtocolVersion || revoke.Reason != "rotated" {
...
func TestServerHTTPHandlerProjectsAgentProcessState(t *testing.T) {
	...
	for {
		...
		if body.Client.Runtime.ConnectionState == ServerClientConnected && body.Client.Runtime.ProcessState == FRPProcessRunning {
			break
		}
		if time.Now().After(deadline) {
	...
	for {
		...
		if body.Client.Runtime.ProcessState == FRPProcessRunning && body.Client.Runtime.LastError != nil && body.Client.Runtime.LastError.Code == "APPLY_FAILED" && body.Client.Runtime.LastError.Revision != nil && *body.Client.Runtime.LastError.Revision == 0 {
			break
		}
		if time.Now().After(deadline) {
...
func TestServerHTTPHandlerRestartsOnlyOwnedOnlineClients(t *testing.T) {
	...
	var frame RestartFRPC
	if err := json.Unmarshal(source, &frame); err != nil {
		t.Fatalf("decode restart frame: %v", err)
	}
	if frame.Type != "restart_frpc" || frame.TunnelProtocolVersion != TunnelProtocolVersion {
...
func TestServerHTTPHandlerStreamsAdministratorFRPSInvalidation(t *testing.T) {
	...
	handler, err := NewServerHTTPHandler(ServerHTTPOptions{
		...
		ServerState: serverHTTPTestStateProvider{state: ServerHTTPState{FRPS: FRPSupervisorState{State: FRPProcessStopped}}},
	})
	if err != nil {
		t.Fatalf("NewServerHTTPHandler() error = %v", err)
	}
