...
func TestParseFRPCTunnelImportMapsCandidates(t *testing.T) {
	...
	if tcp := imported.Candidates[2]; tcp.Input.Protocol != TunnelProtocolTCP || tcp.Input.ServerPort == nil || *tcp.Input.ServerPort != 20001 || tcp.Input.LocalHost == nil || *tcp.Input.LocalHost != "127.0.0.1" {
		t.Fatalf("TCP import candidate = %#v", tcp)
	}
	if udp := imported.Candidates[3]; udp.Input.Protocol != TunnelProtocolUDP || udp.Input.ServerPort == nil || *udp.Input.ServerPort != 20002 {
...
func TestServerControlPlaneImportFRPCTunnelsRejectsSelectionsAndRollsBackReservations(t *testing.T) {
	...
	location := "/api"
	if _, err := plane.CreateTunnel(ctx, client.ID, TunnelMutationInput{Protocol: TunnelProtocolHTTP, CustomDomains: []string{"app.example.com"}, Location: &location, LocalPort: 3000}); err != nil {
