...
func (gateway *ServerAgentGateway) State(clientID string) ServerClientRuntimeState {
	state := ServerClientRuntimeState{
		...
		ProcessState:    FRPProcessStopped,
	}
	if gateway == nil {
		return state
	}
...
func (gateway *ServerAgentGateway) recordProcessState(clientID string, slot uint64, processState FRPProcessState, lastError *StructuredRuntimeError) bool {
	if gateway == nil {
		return false
	}
	gateway.mu.Lock()
	...
	if runtime.processState == "" {
		runtime.processState = FRPProcessStopped
	}
...
func (gateway *ServerAgentGateway) recordRuntimeError(clientID string, slot uint64, lastError *StructuredRuntimeError) bool {
	if gateway == nil {
		return false
	}
	gateway.mu.Lock()
...
func cloneServerAgentRuntimeError(value *StructuredRuntimeError) *StructuredRuntimeError {
	if value == nil {
		return nil
	}
	copy := *value
...
func (gateway *ServerAgentGateway) Authorize(ctx context.Context, authorization string) (*ServerAgentReservation, error) {
	if gateway.frps.FRPSState().State != FRPProcessRunning {
...
func (gateway *ServerAgentGateway) activate(clientID string, slot uint64, connection *ServerAgentConnection) bool {
	...
	if _, found := gateway.runtime[clientID]; !found {
		gateway.runtime[clientID] = serverAgentRuntime{processState: FRPProcessStopped}
	}
	return true
}
