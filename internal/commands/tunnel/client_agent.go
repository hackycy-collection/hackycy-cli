...
func NewClientAgent(options ClientAgentOptions) (*ClientAgent, error) {
	...
	if options.wireTarget != nil {
		...
		resolved, err := CurrentWireTarget()
		if err != nil {
			return nil, err
		}
		target = resolved
...
func decodeClientWelcome(source []byte, expected FRPArtifact) (AgentWelcome, error) {
	var envelope struct {
		Type string `json:"type"`
	}
	if err := json.Unmarshal(source, &envelope); err != nil {
		return AgentWelcome{}, fmt.Errorf("%w: decode control message", ErrClientProtocol)
	}
	if envelope.Type == "incompatible" {
		...
		if err := json.Unmarshal(source, &message); err != nil || strings.TrimSpace(message.Message) == "" {
			return AgentWelcome{}, fmt.Errorf("%w: control plane reported an invalid incompatibility message", ErrClientProtocol)
		}
		return AgentWelcome{}, fmt.Errorf("%w: %s", ErrClientIncompatible, message.Message)
	}
	if envelope.Type != "welcome" {
		return AgentWelcome{}, fmt.Errorf("%w: expected welcome message", ErrClientProtocol)
	}
	var welcome AgentWelcome
	if err := json.Unmarshal(source, &welcome); err != nil {
		return AgentWelcome{}, fmt.Errorf("%w: decode welcome message", ErrClientProtocol)
	}
	if welcome.TunnelProtocolVersion != TunnelProtocolVersion {
		return AgentWelcome{}, fmt.Errorf("%w: Control plane uses an unsupported tunnel protocol; upgrade ycy", ErrClientIncompatible)
	}
	if welcome.RequiredFRPVersion != FRPVersion || welcome.Artifact != expected.Description {
		return AgentWelcome{}, fmt.Errorf("%w: Control plane requires an unsupported tunnel protocol or FRP build; upgrade ycy", ErrClientIncompatible)
	}
	if strings.TrimSpace(welcome.AdvertisedFRPHost) == "" || welcome.AdvertisedFRPPort < 1 || welcome.AdvertisedFRPPort > 65535 || strings.TrimSpace(welcome.InternalFRPToken) == "" || welcome.Snapshot.Revision < 0 || welcome.Snapshot.Revision > clientMaximumSafeInteger {
		return AgentWelcome{}, fmt.Errorf("%w: welcome message is incomplete", ErrClientProtocol)
	}
	return welcome, nil
}
