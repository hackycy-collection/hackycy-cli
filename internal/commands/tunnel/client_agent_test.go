...
func TestClientAgentConnectProbesBeforeV3HelloAndPinnedWelcome(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if writeErr := socket.WriteJSON(AgentWelcome{
			...
			Snapshot:              TunnelSnapshot{ClientKey: "client-id", Revision: 3},
		}); writeErr != nil {
	...
	hello := <-hellos
	if hello.Type != "hello" || hello.TunnelProtocolVersion != TunnelProtocolVersion || hello.YCYVersion != "0.0.0-test" || hello.LastAppliedRevision != 3 || hello.Platform != string(target.Platform) || hello.Architecture != string(target.Architecture) {
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			frame: func(t *testing.T) any {
				...
				return AgentWelcome{
					...
					Snapshot: TunnelSnapshot{ClientKey: "client-id", Revision: 0},
				}
			},
		...
		},
		{
			...
			frame: func(*testing.T) any {
				return Incompatible{Type: "incompatible", TunnelProtocolVersion: TunnelProtocolVersion, Message: "upgrade ycy"}
			},
			want: ErrClientIncompatible,
		},
	} {
