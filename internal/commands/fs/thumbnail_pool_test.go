...
func TestThumbnailWorkerPoolTimeoutKillsWaitsAndReplaces(t *testing.T) {
	...
	if _, err := pool.convert("application/x-thumbnail-test-stall", nil); !serviceErrorIs(err, "THUMBNAIL_TIMEOUT") {
		var service *ServiceError
		if errors.As(err, &service) {
			t.Fatalf("stall error = %v (%v)", err, service.Cause)
		}
...
func TestThumbnailWorkerPoolReplacesCrashedWorker(t *testing.T) {
	...
	if _, err := pool.convert("application/x-thumbnail-test-crash", nil); !serviceErrorIs(err, "THUMBNAIL_WORKER_FAILED") {
...
func TestThumbnailWorkerPoolBoundsQueueAndClosesWorkers(t *testing.T) {
	...
	if _, err := pool.convert("application/x-thumbnail-test-pid", nil); !serviceErrorIs(err, "THUMBNAIL_QUEUE_FULL") {
		t.Fatalf("full queue error = %v", err)
	}
	pool.close()
	for range thumbnailWorkerQueue + 1 {
		if err := <-results; !serviceErrorIs(err, "THUMBNAIL_STOPPED") {
...
func TestThumbnailWorkerPoolRejectsQueueWhenEveryWorkerFails(t *testing.T) {
	...
	defer pool.close()
	if _, err := pool.convert("application/x-thumbnail-test-pid", nil); !serviceErrorIs(err, "THUMBNAIL_WORKER_FAILED") {
