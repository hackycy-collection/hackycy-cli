...
func TestDiscoveryPresenterReceivesHelpButNotVersionOrRawCompletion(t *testing.T) {
	output := &bytes.Buffer{}
	errors := &bytes.Buffer{}
	presenter := &recordingDiscoveryPresenter{}
	app, err := New(BuildInfo{Version: "0.0.0-dev"}, Dependencies{
		Out:       output,
		Err:       errors,
		Discovery: presenter,
	...
	if outcome := app.Execute(context.Background(), []string{"--help"}); outcome.Code != 0 || output.Len() != 0 || errors.Len() != 0 {
		t.Fatalf("help outcome = %#v, stdout = %q, stderr = %q", outcome, output.String(), errors.String())
	}
	if len(presenter.documents) != 1 {
		t.Fatalf("presented documents = %#v, want one", presenter.documents)
	}
	document := presenter.documents[0]
	if document.CommandPath != "ycy" || document.Summary != "Ycy command line interface" || document.Usage != "ycy [flags]" || !containsDiscoveryDescendant(document.Descendants, "export") || !containsDiscoveryFlag(document.Flags, "log-level") {
		t.Fatalf("root document = %#v", document)
	}

	output.Reset()
	if outcome := app.Execute(context.Background(), []string{"--version"}); outcome.Code != 0 || output.String() != "0.0.0-dev\n" || len(presenter.documents) != 1 {
		t.Fatalf("version outcome = %#v, stdout = %q, documents = %#v", outcome, output.String(), presenter.documents)
	}
	output.Reset()
	if outcome := app.Execute(context.Background(), []string{"completion", "bash"}); outcome.Code != 0 || !strings.Contains(output.String(), "__start_ycy") || len(presenter.documents) != 1 {
		t.Fatalf("completion outcome = %#v, stdout = %q, documents = %#v", outcome, output.String(), presenter.documents)
	}
}
...
type recordingDiscoveryPresenter struct {
	documents []DiscoveryDocument
}

func (presenter *recordingDiscoveryPresenter) PresentDiscovery(_ context.Context, document DiscoveryDocument) {
	presenter.documents = append(presenter.documents, document)
}

func containsDiscoveryDescendant(descendants []DiscoveryDescendant, name string) bool {
...
func containsDiscoveryFlag(flags []DiscoveryFlag, name string) bool {
